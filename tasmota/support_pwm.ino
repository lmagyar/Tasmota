...
#ifdef USE_SLOW_PWM
        SlowPWMAnalogWrite(Pin(GPIO_PWM1, i), gpio_val);
#else
...
#ifdef USE_SLOW_PWM
      SlowPWMAnalogWrite(Pin(GPIO_PWM1, pwm_index), bitRead(TasmotaGlobal.pwm_inverted, pwm_index) ? Settings->pwm_range - XdrvMailbox.payload : XdrvMailbox.payload);
#else
...
#ifdef USE_SLOW_PWM
          SlowPWMAnalogWrite(Pin(GPIO_PWM1, i), bitRead(TasmotaGlobal.pwm_inverted, i) ? Settings->pwm_range - Settings->pwm_value_ext[i] : Settings->pwm_value_ext[i]);
#else
